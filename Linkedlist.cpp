...
NODE* createNode(int data)
{
    NODE* p = new NODE;
    if (p == nullptr) {
        std::cout << "khong the tao con tro p" << std::endl;
    }
    p->key = data;
    p->p_next = nullptr;
...
void removeBefore(List*& L, int val)
{
    if (L->p_head != nullptr && L->p_head->key != val) {
        NODE* temp = nullptr;
        NODE* p = L->p_head;
        while (p->p_next!=nullptr&&p->p_next->key != val) {
            temp = p;
            p = p->p_next;
        }
        if (p->p_next != nullptr) {
...
void removeDuplicate(List*& L)
{
    ...
    while (p != nullptr) {
        bool kt = false;
        for (NODE* q = L->p_head; q != p; q = q->p_next) {
            if (q->key == p->key) {
                kt = true;
                break;
            }
